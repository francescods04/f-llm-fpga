...
| W1   | G1d  | A1    | Robust llama.cpp measurement protocol | **FAIL** — llama.cpp main branch and mtp-clean branch both **hang indefinitely** (>5 min timeout) when loading Qwen3.6-35B-A3B MoE. The model is not supported by public llama.cpp builds. | Action: drop llama.cpp for this model. Use **vLLM** as primary GPU backend. Unsloth's claimed 220 tok/s requires their private patched build. |
| W1   | G1e  | A1    | vLLM baseline on target model | **Pending** — need to run `scripts/colab_35b_vllm.py` on A100 or RTX PRO 6000. | Action: run vLLM script and record tok/s + VRAM. This is the true GPU ceiling for Gate G1. |
